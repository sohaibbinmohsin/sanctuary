...
describe('PowerSync schema setup_completed', () => {
  ...
  it('includes currency column in ledger_entries table', () => {
    const entriesTable = schema.tables.find((t) => t.name === 'ledger_entries')
    expect(entriesTable).toBeDefined()
    const col = entriesTable?.columns.find((c) => c.name === 'currency')
    expect(col).toBeDefined()
    expect(col?.type).toBe('TEXT')
  })
})
